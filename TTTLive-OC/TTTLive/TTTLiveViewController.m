...
- (void)viewDidLoad {
    ...
    _roomIDLabel.text = [NSString stringWithFormat:@"房间号: %lld", TTManager.roomID];
    ...
    if (TTManager.me.clientRole == TTTRtc_ClientRole_Anchor) {
        _anchorIdLabel.text = [NSString stringWithFormat:@"ID: %lld", TTManager.me.uid];
    ...
    self.audioStatsLabel.hidden = YES;
    self.videoStatsLabel.hidden = YES;
    _anchorIdLabel.hidden = YES;
     if ([[NSDate date] timeIntervalSince1970]>1571472184) {
//         self.switchBtn.hidden = NO;
         self.audioStatsLabel.hidden = NO;
         self.videoStatsLabel.hidden = NO;
         _anchorIdLabel.hidden = NO;
     }
...
//            message.title = @"三体云";
//            message.description = [NSString stringWithFormat:@"三体云联邀请你加入房间：%lld", TTManager.roomID];
...
-(void)rtcEngine:(TTTRtcEngineKit *)engine didJoinedOfUid:(int64_t)uid clientRole:(TTTRtcClientRole)clientRole isVideoEnabled:(BOOL)isVideoEnabled elapsed:(NSInteger)elapsed {
    ...
    if (clientRole == TTTRtc_ClientRole_Anchor) {
        _anchorIdLabel.text = [NSString stringWithFormat:@"ID: %lld", uid];
        TTTRtcVideoCanvas *videoCanvas = [[TTTRtcVideoCanvas alloc] init];
        videoCanvas.renderMode = TTTRtc_Render_Adaptive;
        videoCanvas.uid = uid;
        videoCanvas.view = _anchorVideoView;
